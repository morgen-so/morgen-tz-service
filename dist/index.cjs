...
var Timezone = class _Timezone {
  ...
  getIanaTzName(tzName) {
    if (!tzName) {
      return _Timezone.findCanonicalIANAName(import_moment_timezone.default.tz.guess());
    }
    tzName = tzName.trim();
    tzName = _Timezone.findCanonicalIANAName(tzName);
    if (import_moment_timezone.default.tz.zone(tzName)) {
    ...
    if (ianaInferred) {
      ...
      if (import_moment_timezone.default.tz.zone(ianaInferredCanonical)) {
        this.validated_ = true;
   ...
   */
  bestGuess(tzName) {
    const names = import_moment_timezone.default.tz.names();
