...
class Timezone {
    ...
    static findCanonicalIANAName(ianaName) {
        if (canonicalNames[ianaName])
            return canonicalNames[ianaName];
        return ianaName;
    }
    /**
    ...
    getIanaTzName(tzName) {
        ...
        if (ianaInferred) {
            const ianaInferredCanonical = Timezone.findCanonicalIANAName(ianaInferred);
        ...
        console.error(`Could not map timezone "${tzName}" to an IANA timezone database name.`);
        return Timezone.findCanonicalIANAName(moment.tz.guess());
    }
