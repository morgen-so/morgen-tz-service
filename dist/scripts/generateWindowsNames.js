...
(async function () {
    ...
    const xmlData = fs.readFileSync(path.join(path.resolve(), "./data/windowsZones.xml"), "utf-8");
    ...
    const zones = data["supplementalData"]["windowsZones"]["mapTimezones"]["mapZone"];
    ...
    fs.writeFileSync("./src/generated/windowsZones.ts", `${header}export const windowsZones: Record<string, string> = ${JSON.stringify(windowsZones, null, 2)}`);
    ...
    const windowsZonesInverse = _(zones)
        ...
        .fromPairs()
        .toJSON();
    fs.writeFileSync("./src/generated/windowsZonesInverse.ts", `${header}export const windowsZonesInverse: Record<string, string> = ${JSON.stringify(windowsZonesInverse, null, 2)}`);
})();
//# sourceMappingURL=generateWindowsNames.js.map